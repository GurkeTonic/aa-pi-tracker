...
def create_project(user, name, assignments, *, allowed_owner_ids=None):
    ...
    if allowed_owner_ids is not None:
        planets_by_pk = {
            p.pk: p for p in PiPlanet.objects.filter(pk__in=pks, owner_id__in=allowed_owner_ids)
        }
    else:
        planets_by_pk = {p.pk: p for p in PiPlanet.objects.filter(pk__in=pks, owner__user=user)}

    rows = []
    for a in normalized:
        if a.get("pk") and a["pk"] in planets_by_pk:
            rows.append(PiProjectPlanet(
                project=project,
